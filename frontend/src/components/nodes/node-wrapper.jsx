...
export const NodeWrapper = ({ title, children, handles, id, icon: Icon }) => {
  ...
  const getLabelStyle = (position) => {
    switch (position) {
      ...
      case Position.Right:
        return "absolute transform translate-x-[16px] top-2";
      case Position.Top:
  ...
  return (
    <Card className="group relative w-[250px] p-3 border-none rounded-xl bg-[#FCF259] shadow-2xl shadow-black/30 transition-all">
      <button
        onClick={() => removeNode(id)}
        className="absolute top-2 right-2 text-gray-800 hover:text-red-500 transition-colors"
      >
        <XCircle size={18} />
      </button>
      {handles.map((handle) => (
        <Handle
          ...
          className="relative !w-4 !h-4 !bg-[#bf6bf7] hover:!bg-[#80D8C3] transition-colors"
        >
          {handle.label && (
            <span
